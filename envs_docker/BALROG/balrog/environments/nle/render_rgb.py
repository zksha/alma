...
def load_atlas():
    ...
    with open(tiles_path, "rb") as f:
        tiles = pickle.load(f)
    N = len(tiles)
    return np.stack([tiles[i] for i in range(N)])
...
def rgb_render_image(glyphs, *, texture_atlas=None):
    ...
    nrows, ncols = glyphs.shape
    tiles = glyph2tile[glyphs]
    assert tiles.max() < MAXOTHTILE
    return (
        texture_atlas[tiles]
        .transpose(0, 2, 1, 3, 4)
        .reshape(nrows * texture_atlas.shape[1], ncols * texture_atlas.shape[2], 3)
    )
...
if __name__ == "__main__":
    ...
    Image.fromarray(rgb_render_image(glyphs)).save("test.png")
