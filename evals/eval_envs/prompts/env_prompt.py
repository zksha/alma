...
intruction_prompts = dict(
    ...
    You have 80 steps to complete the task. Restarting is forbidden. Please out put the command without explaination.
    """,
    coin_collector="""
    You are an agent playing TextWorld, a text-based adventure game where you are in a randomly generated
    ...
    room.
    You have 25 steps to complete the task. Restarting is forbidden. Please out put the command without explaination.
    """,
)


def get_textworld_prompt(task_type: str, obs: str, memory_retrived: Dict = {}, goal = '',  **kargs):
    if memory_retrived:
        memory_prompt = f"""
    **Relevant Experience: **
    ...
    instruction_prompt = intruction_prompts[task_type].strip() + goal + memory_prompt
